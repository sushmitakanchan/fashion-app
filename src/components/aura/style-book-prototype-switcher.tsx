...
import {
  STYLE_BOOK_REVIEW_VARIANTS,
  showPrototypeControls,
  stepStyleBookReviewVariant,
  type StyleBookReviewVariant,
} from "@/lib/style-book-review-prototype-state";
...
/** Development-only control for judging the three throwaway page structures. */
export function StyleBookPrototypeSwitcher({
  current,
}: {
  ...
  const previous = React.useCallback(
    () => select(stepStyleBookReviewVariant(current, -1)),
    [current, select],
  );
  ...
  const active = STYLE_BOOK_REVIEW_VARIANTS.find(
    (variant) => variant.key === current,
  )!;

  return (
    <aside
      aria-label="Style Book prototype switcher"
