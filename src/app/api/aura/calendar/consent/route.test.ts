...
/**
 * Route-level tests for Smart Planning consent (grant / withdraw / read). A
 * store-backed Prisma stub lets grant-then-read and withdraw-then-read be
 * observed end to end, so "records the consenting moment and policy version" and
 * "withdrawal only bars future planning" are testable as real state. Mirrors the
 * wardrobe-analysis consent route test.
 */
...
const get = () => GET();
const grant = (policyVersion: number = PLANNING_POLICY_VERSION) =>
  POST(
    new Request("http://localhost/api/aura/calendar/consent", {
      method: "POST",
...
describe("planning consent — read", () => {
...
describe("planning consent — grant", () => {
  it("records the moment and policy version, then reads back active", async () => {
  ...
  it("refuses a stale disclosure (policy version mismatch)", async () => {
    const response = await grant(PLANNING_POLICY_VERSION + 999);
    expect(response.status).toBe(400);
    // Nothing recorded.
    expect(consents.u1).toBeUndefined();
  });

  it("rejects a malformed (non-integer) policy version", async () => {
    const response = await POST(
      new Request("http://localhost/api/aura/calendar/consent", {
        method: "POST",
        body: JSON.stringify({ policyVersion: "1" }),
      }),
    );
    expect(response.status).toBe(400);
    expect(consents.u1).toBeUndefined();
...
describe("planning consent — withdraw (future only)", () => {
