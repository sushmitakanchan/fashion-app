...
export function WardrobeReviewPrototype({ requestedVariant }: { requestedVariant?: string }) {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8 sm:py-12">
      ...
      <style>{`
        button.group > span:last-child { display: none; }
        button.group > span:first-child > span:first-child {
          box-shadow: 0 4px 0 rgb(20 17 15 / 0.45);
          transition: transform 150ms ease, box-shadow 150ms ease;
        }
        button.group:active > span:first-child > span:first-child {
          transform: translateY(3px);
          box-shadow: 0 1px 0 rgb(20 17 15 / 0.45);
        }
      `}</style>
...
function FocusQueue({ items, active, setActiveId, update, onRetryUpload }: { items: Item[]; active: Item; setActiveId: (id: string) => void; update: (id: string, changes: Partial<Item>) => void; onRetryUpload: () => void }) {
  ...
  const failedItem = items.find((item) => item.status === "failed");
  return <section className="mx-auto max-w-5xl"><div className="mb-5 grid gap-3 sm:grid-cols-3"><button type="button" onClick={() => firstReady && setActiveId(firstReady.id)} className="rounded-xl border border-emerald-500/35 bg-emerald-500/5 p-4 text-left transition hover:bg-emerald-500/10 focus-visible:ring-3 focus-visible:ring-ring focus-visible:outline-none"><p className="text-xs font-bold tracking-[0.14em] uppercase text-muted-foreground">High confidence</p><p className="font-heading mt-1 text-2xl uppercase">{confirmed} ready</p><p className="mt-1 text-xs text-muted-foreground">Open {firstReady?.name ?? "a ready item"}.</p></button><button type="button" onClick={() => firstReview && setActiveId(firstReview.id)} className="rounded-xl border border-amber-500/45 bg-amber-500/5 p-4 text-left transition hover:bg-amber-500/10 focus-visible:ring-3 focus-visible:ring-ring focus-visible:outline-none"><p className="text-xs font-bold tracking-[0.14em] uppercase text-muted-foreground">Your attention</p><p className="font-heading mt-1 text-2xl uppercase">{needsReview} calls</p><p className="mt-1 text-xs text-muted-foreground">Open {firstReview?.name ?? "an uncertain item"}.</p></button><button type="button" onClick={() => failedItem && setActiveId(failedItem.id)} className="rounded-xl border border-destructive/35 bg-destructive/5 p-4 text-left transition hover:bg-destructive/10 focus-visible:ring-3 focus-visible:ring-ring focus-visible:outline-none"><p className="text-xs font-bold tracking-[0.14em] uppercase text-muted-foreground">Import health</p><p className="font-heading mt-1 text-2xl uppercase">{failed} retry</p><p className="mt-1 text-xs text-muted-foreground">Open {failedItem?.name ?? "the failed item"}.</p></button></div><div className="rounded-2xl border bg-card p-5 sm:p-7"><div className="grid gap-7 sm:grid-cols-[minmax(12rem,20rem)_1fr]"><ItemPhoto item={active} /><div><ReviewBadge item={active} /><h2 className="font-heading mt-3 text-3xl uppercase">{active.name}</h2><p className="mt-2 text-sm text-muted-foreground">AI filled these fields from the photo. Correct anything before moving on.</p><div className="mt-6 grid gap-4">{active.status === "failed" ? <button type="button" onClick={onRetryUpload} className="group flex items-center justify-between gap-4 rounded-xl border-2 border-dashed border-brand-magenta/45 bg-brand-magenta/5 p-3 text-left transition hover:border-brand-magenta hover:bg-brand-magenta/10 focus-visible:ring-3 focus-visible:ring-ring focus-visible:outline-none"><span className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-full bg-brand-magenta text-white"><UploadIcon className="size-4" /></span><span><span className="block text-sm font-bold">Replace failed photo</span><span className="block text-xs text-muted-foreground">Upload a new image before categorising.</span></span></span><span className="rounded-full bg-foreground px-3 py-2 text-xs font-bold text-background transition group-hover:scale-105">Choose image</span></button> : null}<Field label="Category" value={active.category} placeholder="Choose category" onChange={(category) => update(active.id, { category })} /><Field label="Colour" value={active.colour} placeholder="Add colour" onChange={(colour) => update(active.id, { colour })} /><Field label="Brand (optional)" value={active.brand} placeholder="Leave blank if unknown" onChange={(brand) => update(active.id, { brand })} /></div></div></div><div className="mt-7 flex items-center justify-between border-t pt-5"><button type="button" onClick={() => adjacent(-1)} className="inline-flex items-center gap-1 text-sm font-bold"><ArrowLeftIcon className="size-4" /> Previous</button><span className="text-xs text-muted-foreground">Review {index + 1} of {items.length}</span><button type="button" onClick={() => adjacent(1)} className="inline-flex items-center gap-1 text-sm font-bold">Next <ArrowRightIcon className="size-4" /></button></div></div></section>;
}
